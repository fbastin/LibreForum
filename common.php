    ...
    if (! include_once( "./include/db/config.php" )) {
        ...
        if (!file_exists("./include/db/config.php")) { ?>
            ...
            if (!$fp) { ?>
                ...
                ./include/db/config.php,<br/>but LibreForum was unable to read it.
...
$PHORUM['cache'] = './cache';
$PHORUM['template_http_path'] = (isset($PHORUM['http_path']) ? $PHORUM['http_path'] : '').'/templates';
...
// magic_quotes removed in PHP 8.0 — no action needed.
...
function phorum_ob_clean()
{
    ...
    for(;;) {
        ...
        if (!$status || $status['name'] == 'ob_gzhandler') break;
        // PHP 8+ replaced 'del' with 'flags'; check both for compatibility.
        $deletable = isset($status['del'])
            ? $status['del']
            : (($status['flags'] ?? 0) & PHP_OUTPUT_HANDLER_REMOVABLE);
        if (!$deletable) break;
        ob_end_clean();
    }
}
