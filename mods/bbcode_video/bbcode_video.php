...
function phorum_mod_bbcode_video_format($data)
{
    ...
    $search[] = "/\[video\]https?:\/\/youtu.be\/([\w_-]+)\[\/video\]/is";
    $replace[] = "[video]youtube:$1[/video]";

    // Handle Google [video] URLs.
    $search[] = "/\[video\]https?:\/\/(video.google.[a-z]+)\/.*?docid=(-?\d+)[a-z0-9;\/\?:@=\&\$\-_\.\+!*'\(\),~%#]*\[\/video\]/is";
    $replace[] = "[video]google:$1:$2[/video]";

    // Handle Vimeo [video] URLs.
    $search[] = "/\[video\]https?:\/\/(?:www.)?vimeo.com\/(\d+)\[\/video\]/";
    $replace[] = "[video]vimeo:$1[/video]";

    // Handle break.com URLs.
    $search[] = "/\[video\]https?:\/\/embed.break.com\/(\w+)\[\/video\]/";
    $replace[] = "[video]break:$1[/video]";

    // Handle ebaumsworld.com URLs.
    $search[] = "/\[video\]https?:\/\/(?:www\.)?ebaumsworld\.com\/(\d\d\d\d\/\d\d\/[\w\.-]+\.flv)\[\/video\]/";
    ...
    if (!empty($PHORUM["mod_bbcode_video"]["handle_plain_urls"]))
    {
        ...
        $search[] = "/https?:\/\/youtu.be\/([\w_-]+)/is";
        $replace[] = "[video]youtube:$1[/video]";

        // Google URLs.
        $search[] = "/\[url\]https?:\/\/(video.google.[a-z]+)\/.*?docid=(-?\d+)[a-z0-9;\/\?:@=\&\$\-_\.\+!*'\(\),~%#]*\[\/url\]/is";
        $replace[] = "[video]google:$1:$2[/video]";
        $search[] = "/https?:\/\/(video.google.[a-z]+)\/.*?docid=(-?\d+)[a-z0-9;\/\?:@=\&\$\-_\.\+!*'\(\),~%#]*/is";
        $replace[] = "[video]google:$1:$2[/video]";

        // Vimeo URLs.
        $search[] = "/\[url\]https?:\/\/(?:www.)?vimeo.com\/(\d+)\[\/url\]/";
        $replace[] = "[video]vimeo:$1[/video]";
        $search[] = "/https?:\/\/(?:www.)?vimeo.com\/(\d+)/";
        $replace[] = "[video]vimeo:$1[/video]";

        // break.com URLs.
        $search[] = "/\[url\]https?:\/\/embed.break.com\/(\w+)\[\/url\]/";
        $replace[] = "[video]break:$1[/video]";
        $search[] = "/https?:\/\/embed.break.com\/(\w+)\b/";
        $replace[] = "[video]break:$1[/video]";
        ...
        $search[] = "/\[url\]https?:\/\/(?:www\.)?ebaumsworld\.com\/(\d\d\d\d\/\d\d\/[\w\.-]+\.flv)\[\/url\]/";
        $replace[] = "[video]ebaumsworld:$1[/video]";
        $search[] = "/https?:\/\/(?:www\.)?ebaumsworld\.com\/(\d\d\d\d\/\d\d\/[\w\.-]+\.flv)\b/";
        $replace[] = "[video]ebaumsworld:$1[/video]";
    }
    ...
    $replace[] = "<br/><embed src=\"http://$1/googleplayer.swf?docId=$2\" type=\"application/x-shockwave-flash\" width=\"425\" height=\"350\"></embed><br/>";

    // Final replacement for Vimeo videos.
    $search[] = "/\[video\]vimeo:(\d+)\[\/video\]/is";
    $replace[] = "<br/><embed src=\"http://www.vimeo.com/moogaloop.swf?clip_id=$1&amp;server=vimeo.com&amp;fullscreen=1\" type=\"application/x-shockwave-flash\" width=\"425\" height=\"350\"></embed><br/>";

    // Final replacement for break.com videos.
    $search[] = "/\[video\]break:(\w+)\[\/video\]/is";
    $replace[] = "<br/><embed src=\"http://embed.break.com/$1\" type=\"application/x-shockwave-flash\" width=\"464\" height=\"392\"></embed><br/>";

    // Final replacement for ebaumsworld.com videos.
    $search[] = "/\[video\]ebaumsworld:(\d\d\d\d\/\d\d\/[\w\.-]+)\.flv\[\/video\]/is";
    $replace[] = "<br/><embed src=\"http://www.ebaumsworld.com/mediaplayer.swf\" flashvars=\"file=/$1.flv&displayheight=321&image=/$1.jpg\" loop=\"false\" menu=\"false\" quality=\"high\" bgcolor\"#ffffff\" width=\"425\" height=\"345\" type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" /></br>";
