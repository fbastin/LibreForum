 ...
 */
function mod_openstreetmap_build_maptool($maptool_type, $state = NULL)
{
    global $PHORUM;
    ...
    if (! isset($PHORUM['DATA']['LANG']['mod_openstreetmap'])) {
    ...
    foreach ($PHORUM['mod_openstreetmap'] as $key => $val) {
    ...
    $PHORUM['maptool']['api_language'] =
        $PHORUM["DATA"]["LANG"]["mod_openstreetmap"]["geocoding_lang"];
    ...
    $parameters = array(
        PHORUM_ADDON_URL, "module=openstreetmap", "addon=mapframe"
    );
    ...
    $PHORUM['maptool']['lang'] = $PHORUM["DATA"]["LANG"]["mod_openstreetmap"];
 ...
 */
function mod_openstreetmap_filter_state_data($data)
{
    $filtered = array();
 ...
 * @return array
 */
function mod_openstreetmap_upgrade_userdata($d)
{
    if (!empty($d['center'])) {
        if (preg_match(
            '/^\((-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\)$/',
