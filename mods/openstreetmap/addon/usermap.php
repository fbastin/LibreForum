...
$PHORUM['DATA']['HEADING'] =
    $PHORUM['DATA']['LANG']['mod_openstreetmap']['UserMapTitle'];
...
// Collect all users which have a "mod_openstreetmap" custom user profile field.
$field = phorum_api_custom_profile_field_byname('mod_openstreetmap');
if (empty($field)) trigger_error(
    'No custom profile field named "mod_openstreetmap" available',
    E_USER_ERROR
);
...
foreach ($users as $user)
{
    ...
    if (isset($PHORUM['hooks']['openstreetmap_user_info']))
    {
        ...
        $plot = phorum_hook('openstreetmap_user_info', $plot);
    }
...
if (count($show) == 0) {
    phorum_output('openstreetmap::usermap_nousers');
    exit();
}

// Build HTML code for the map.
$PHORUM['DATA']['MOD_OPENSTREETMAP'] =
    mod_openstreetmap_build_maptool('plotter', $show);

// Display the user map.
phorum_output('openstreetmap::usermap');
