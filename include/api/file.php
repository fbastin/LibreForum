    ...
    if ($file["mime_type"] === NULL)
    {
        ...
        if(function_exists("finfo_open") &&
           ...
           $finfo = @finfo_open(FILEINFO_MIME,$mime_magic_file)) {
            ...
            $detected = finfo_buffer($finfo,$file['file_data']);
            finfo_close($finfo);
            if ($detected === FALSE) return phorum_api_error_set(
                PHORUM_ERRNO_ERROR,
                "The mime-type of file {$file["file_id"]} couldn't be determined through the" .
                "fileinfo-extension"
            );
            // finfo with FILEINFO_MIME returns "type/subtype; charset=…";
            // strip the charset part so comparisons work correctly.
            $file["mime_type"] = preg_replace('/\s*;.*$/', '', $detected);
