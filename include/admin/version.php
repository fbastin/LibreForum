...
// Check for a new version of the LibreForum software. If there's a new version,
...
print '<div class="LibreForumAdminTitle">Check for new LibreForum version</div>';
print '<br/>';
...
if ($running_type == 'snapshot') {
    print "<br/>If this LibreForum installation is run on a production server, " .
          "the LibreForum team recommends upgrading to either a stable " .
...
if ($running_type == 'stable' && $new_s) {
    print "<br/>A new stable release is available. The LibreForum team " .
...
if ($running_type == 'development' && $new_s && $new_d) {
    print "<br/>There's both a new stable and a new development release " .
          "available. If this LibreForum installation " .
          "is run on a production server, the LibreForum team recommends " .
...
if ($running_type == 'development' && $new_d && ! $new_s) {
    print "<br/>A new development release is available. If this LibreForum " .
          "installation is run on a production server, the LibreForum team " .
...
if (! $found_upgrade) {
    print "<br/><h3 class=\"input-form-th\">" .
          "Your LibreForum installation is up to date</h3>";
}
?>
